...
|[makefile](./Makefile) | Makefile for setting up the sample app. The `launch-vm` target provisions an Ubuntu 24.04 VM. There is a minimal interface contract that must be obeyed, which the Github Actions CI expects to be present, which you can find documented briefly under [section 6 here](https://github.com/grafana/integration-sample-apps/blob/main/sample-apps/README.md) |
