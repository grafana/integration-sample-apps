...
This sample application creates an Ubuntu VM integrated with Alloy for metric and log collection. This sample app utilizes cloud-init and Make commands to facilitate the setup, configuration, and monitoring of RabbitMQ using the built-in Prometheus metrics collector.
...
1. **Clone the repository**: 
   ...
   cd integration-sample-apps/rabbitmq-node
   ```
...
4. **Create and set up VMs**: 
   Use `make run` to start the rabbitmq sample app.
...
- `make run`: Creates the rabbitmq sample app.
...
## Validating services
### RabbitMQ
- **Check service status**: Confirm that RabbitMQ is running.
  ```bash
